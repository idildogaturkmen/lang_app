...
class SupabaseDB:
    ...
    def add_vocabulary(self, word_original, word_translated, language_translated, category=None, image_path=None):
        """Add vocabulary to Supabase with enhanced error handling."""
        user_id = self.get_user_id()
        if not user_id:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if result and len(result) > 0:
                    ...
                    print("❌ No data returned from successful request")
                    return None
            else:
...
def save_image_to_supabase(image, label, detection_bbox=None):
    ...
    try:
        ...
        img_byte_arr = io.BytesIO()
        processed_image.convert('RGB').save(img_byte_arr, format='JPEG', quality=85)
        img_byte_arr.seek(0)
