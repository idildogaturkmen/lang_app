...
def get_supabase_user_id():
    """Get properly formatted user ID for Supabase operations."""
...
class GamificationSystem:
    def __init__(self, db_path="language_learning.db", translate_func=None, db_instance=None):
        self.db_path = db_path
        self.translate_func = translate_func
        self.supabase_db = db_instance  # Store the SupabaseDB instance
    ...
    def save_game_state(self):
        ...
        try:
            user_id = get_supabase_user_id()
            if not user_id:
                print("❌ No valid user ID found for saving game state")
                return
            
            # Save streak data to Supabase
            self._save_streak_data_to_supabase(user_id)
            ...
            try:
                ...
                SupabaseDB = getattr(main_module, 'SupabaseDB')
                db = SupabaseDB()
                ...
                print(f"❌ Error saving game state to Supabase: {e}")
                # Fallback to local storage
                self._save_to_local_db()
            
        except Exception as e:
            print(f"❌ Error in save_game_state: {e}")
            # Fallback to local storage
            self._save_to_local_db()
    
    def _save_streak_data_to_supabase(self, user_id):
        """Save streak data to Supabase user_streaks table."""
        try:
            if not self.supabase_db:
                print("❌ No Supabase DB instance available")
                return False
            ...
            response = self.supabase_db.supabase.table('user_streaks').upsert(
                streak_data,
                on_conflict='user_id'
            ).execute()
    ...
    def _save_to_local_db(self):
        """Fallback method to save to local SQLite database."""
        try:
            ...
            conn.commit()
            conn.close()
            print(f"✅ Game state saved to local database as fallback")
            
        except Exception as e:
            print(f"❌ Error saving to local database: {e}")

    def load_game_state(self):
        """Load game state from Supabase with local fallback."""
        try:
            user_id = get_supabase_user_id()
            if not user_id:
                print("❌ No valid user ID found, loading from local database")
                return self._load_from_local_db()
            
            # Load streak data from Supabase
            self._load_streak_data_from_supabase(user_id)
            
            # Load other game state from Supabase
            try:
                # Lazy import to avoid circular import
                import importlib
                main_module = importlib.import_module('main')
                ...
                if response.data and len(response.data) > 0:
                    ...
                    print(f"🆕 No existing game state in Supabase, using defaults")
                    return False
            except Exception as e:
            ...
            print(f"❌ Error in load_game_state: {e}")
            return self._load_from_local_db()

    def _load_streak_data_from_supabase(self, user_id):
        """Load streak data from Supabase."""
        try:
            # Lazy import to avoid circular import
            import importlib
            ...
            response = db.supabase.table('user_streaks').select('*').eq('user_id', user_id).execute()
    ...
    def _load_from_local_db(self):
        """Fallback method to load from local SQLite database."""
        try:
            # Connect to the database
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            print(f"✅ Game state loaded from local database")
            return True
        except Exception as e:
    ...
    def get_translation_for_word(self, word, target_language):
        ...
        common_translations = {
            ...
            "ja": {
                ...
                "sorry": "すみません", "excuse me": "すみません", "love": "愛", 
